...
export async function startConnectOnboarding(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    // Sensible defaults for dev if the mobile client doesn't pass them
    const refresh = refreshUrl || 'peerdeliver://stripe/refresh';
    const returnTo = returnUrl || 'peerdeliver://stripe/return';
    const url = await paymentService.createOnboardingLink(req.user!.userId, refresh, returnTo);
    success(res, { url });
  } catch (err) {
    next(err);
  }
